...
#define CPLD_I2C_RETRY_WAIT_TIME 10     /* 延时10ms再操作 */
...
static ssize_t show_vdd_value(struct device *dev, struct device_attribute *da, char *buf)
{
    ...
    if (current_vol_index == ref_vol_index) { /* 当前电压为参考电压 */
...
static SENSOR_DEVICE_ATTR(qsfp_reset, S_IRUGO| S_IWUSR, show_cpld_sysfs_value, set_cpld_sysfs_value, 0xb9);
