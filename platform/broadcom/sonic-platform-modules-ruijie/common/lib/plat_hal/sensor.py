    ...
    def Max(self):
        try:
            if isinstance(self.Max_config, dict):
                ret, val = self.get_median(self.Max_config, self.read_times)
                if ret is False or val is None:
                    return None
            else:
                val = self.Max_config    
            if self.format is None:
                ...
                self.__Max = eval(self.format % val)
    ...
    def High(self):
        try:
            if isinstance(self.High_config, dict):
                ret, val = self.get_median(self.High_config, self.read_times)
                if ret is False or val is None:
                    return None
            else:
                val = self.High_config    
            if self.format is None:
                self.__High = self.High_config
            else:
                self.__High = eval(self.format % val)
