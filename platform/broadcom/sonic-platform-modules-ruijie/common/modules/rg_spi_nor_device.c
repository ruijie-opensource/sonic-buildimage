 ...
 *
 * rg_spi_nor_device.c
 * Original Author: sonic_rd@ruijie.com.cn 2021-02-05
 */
...
int g_rg_spi_nor_dev_debug = 0;
int g_rg_spi_nor_dev_error = 0;
int spi_bus_num = DEFAULT_SPI_BUS_NUM;

module_param(g_rg_spi_nor_dev_debug, int, S_IRUGO | S_IWUSR);
module_param(g_rg_spi_nor_dev_error, int, S_IRUGO | S_IWUSR);
module_param(spi_bus_num, int, S_IRUGO | S_IWUSR);

#define SPI_NOR_DEV_DEBUG_VERBOSE(fmt, args...) do {                                        \
    if (g_rg_spi_nor_dev_debug) { \
        printk(KERN_INFO "[SPI_NOR_DEV][VER][func:%s line:%d]\r\n"fmt, __func__, __LINE__, ## args); \
    } \
} while (0)

#define SPI_NOR_DEV_DEBUG_ERROR(fmt, args...) do {                                        \
    if (g_rg_spi_nor_dev_error) { \
...
static int __init rg_spi_nor_dev_init(void)
{
...
static void __exit rg_spi_nor_dev_exit(void)
{
...
module_init(rg_spi_nor_dev_init);
module_exit(rg_spi_nor_dev_exit);
