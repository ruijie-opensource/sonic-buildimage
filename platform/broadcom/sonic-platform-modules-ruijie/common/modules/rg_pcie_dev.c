...
static int pci_dev_probe(struct platform_device *pdev)
{
    ...
    if (pdev->dev.of_node) {
        ...
        rg_pci_dev->search_mode = SEARCH_DEV_DEFAULT;
        of_property_read_u32(pdev->dev.of_node, "search_mode", &rg_pci_dev->search_mode);
        ret = 0;
        if (rg_pci_dev->search_mode == SEARCH_DEV_BY_BRIDGE) {
            ret += of_property_read_u32(pdev->dev.of_node, "bridge_bus", &rg_pci_dev->bridge_bus);
            ret += of_property_read_u32(pdev->dev.of_node, "bridge_slot", &rg_pci_dev->bridge_slot);
            ret += of_property_read_u32(pdev->dev.of_node, "bridge_fn", &rg_pci_dev->bridge_fn);
            if (ret != 0) {
                ...
                PCIE_DEV_DEBUG_VERBOSE("bridge_bus:0x%02x, bridge_slot:0x%02x, bridge_fn:0x%02x.\n",
                    rg_pci_dev->bridge_bus, rg_pci_dev->bridge_slot, rg_pci_dev->bridge_fn);
            }
        } else {
        ...
        rg_pci_dev->search_mode = pci_dev_device->search_mode;
        rg_pci_dev->bridge_bus = pci_dev_device->bridge_bus;
        rg_pci_dev->bridge_slot = pci_dev_device->bridge_slot;
        rg_pci_dev->bridge_fn = pci_dev_device->bridge_fn;
    ...
    PCIE_DEV_DEBUG_VERBOSE("name:%s, domain:0x%04x, bus:0x%02x, slot:0x%02x, fn:%u, bar:%u, bus_width:%d, search_mode:%d.\n",
        ...
        rg_pci_dev->bar, rg_pci_dev->bus_width, rg_pci_dev->search_mode);

    if (rg_pci_dev->search_mode == SEARCH_DEV_DEFAULT) {
    ...
    } else if (rg_pci_dev->search_mode == SEARCH_DEV_BY_BRIDGE) {
        devfn = PCI_DEVFN(rg_pci_dev->bridge_slot, rg_pci_dev->bridge_fn);
        pci_bridge_dev = pci_get_domain_bus_and_slot(rg_pci_dev->domain, rg_pci_dev->bridge_bus, devfn);
        if (pci_bridge_dev == NULL) {
            dev_err(&pdev->dev, "Failed to find pci_bridge_dev, domain:0x%04x, bus:0x%02x, devfn:0x%x\n",
                rg_pci_dev->domain, rg_pci_dev->bridge_bus, devfn);
            return -ENXIO;
        }
