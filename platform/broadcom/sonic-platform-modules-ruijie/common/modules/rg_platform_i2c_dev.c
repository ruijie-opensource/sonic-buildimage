...
#include "rg_platform_i2c_dev.h"

#define PROXY_NAME "rg-platform-i2c-dev"
...
static int transfer_read(struct platform_i2c_dev_info *i2c_dev, u8 *buf, loff_t regaddr, size_t count)
{
    ...
    i = 0;
    /* 分配位宽大小的buf，用以存放寄存器地址 */
    memset(offset_buf, 0, sizeof(offset_buf));
    /* 按地址位宽决定寄存器地址位数，例如4字节位宽发送4字节寄存器地址 */
    ...
    if (adap->algo->master_xfer) {
        ...
        msgs[0].flags = 0;
        msgs[0].len = i2c_dev->addr_bus_width;  /* mesg[0]长度为寄存器地址长度 */
        msgs[0].buf = offset_buf;

        msgs[1].addr = i2c_dev->i2c_addr;
        msgs[1].flags = I2C_M_RD;
        msgs[1].len = count;                    /* mesg[1]长度为数据长度 */
        msgs[1].buf = buf;
...
static int transfer_write(struct platform_i2c_dev_info *i2c_dev, u8 *buf, loff_t regaddr, size_t count)
{
    ...
    if (adap->algo->master_xfer) {
        ...
        msgs[0].addr = i2c_dev->i2c_addr;
        msgs[0].flags = 0;
        msgs[0].len = i2c_dev->addr_bus_width + count;                    /* len长度为寄存器地址长度加数据长度 */
        msgs[0].buf = offset_buf;
...
static int device_read(struct platform_i2c_dev_info *i2c_dev, uint32_t offset, uint8_t *buf, int count)
{
    ...
    /* 数据转换，内容需要转换按位宽为单位存储，1字节位宽直接拷贝，其他需要处理 */
    /* 每个位宽数据内部是先高位再低位。因此需要将每个位宽内数据转换成先低位再高位给上层 */
...
static int device_write(struct platform_i2c_dev_info *i2c_dev, uint32_t offset, uint8_t *buf, size_t count)
{
    ...
    memset(val, 0, sizeof(val));
    /* 数据转换，内容需要转换按位宽为单位存储，1字节位宽直接拷贝，其他需要处理 */
    /* 每个位宽数据内部是先高位再低位。因此需要将上层数据转换(上层数据是每个位宽内也是先低位再高位) */
    if (width == WIDTH_1Byte) {
...
static ssize_t i2c_dev_read(struct file *file, char __user *buf, size_t count, loff_t *offset)
{
    ...
    if (copy_to_user(buf, val, count)) {
        ...
        *offset += count;                               /* 读完数据偏移地址增加count字节 */
    }
...
static ssize_t i2c_dev_write(struct file *file, const char __user *buf, size_t count, loff_t *offset)
{
    ...
    *offset += count;   /* 写完count地址增加count字节 */
    return count;
}
...
static const struct of_device_id platform_i2c_dev_of_match[] = {
    { .compatible = "ruijie,rg-platform-i2c-dev" },
    { },
};
MODULE_DEVICE_TABLE(of, platform_i2c_dev_of_match);

static struct platform_driver rg_platform_i2c_dev_driver = {
...
static int __init rg_platform_i2c_dev_init(void)
{
    return platform_driver_register(&rg_platform_i2c_dev_driver);
}

static void __exit rg_platform_i2c_dev_exit(void)
{
    platform_driver_unregister(&rg_platform_i2c_dev_driver);
}

module_init(rg_platform_i2c_dev_init);
module_exit(rg_platform_i2c_dev_exit);
