...
{
	const struct pmbus_driver_info *info = rg_pmbus_get_driver_info(client);
	long val;
	s16 exponent;
	s32 mantissa;
	int ret;
	...
	case PMBUS_VOUT_UV_FAULT_LIMIT:
		ret = rg_pmbus_read_word_data(client, page, reg);
		if (ret < 0)
			return ret;
...
static const struct of_device_id __maybe_unused xdpe122_of_match[] = {
	{.compatible = "ruijie,rg_xdpe12254"},
	{.compatible = "ruijie,rg_xdpe12284"},
	{}
};
