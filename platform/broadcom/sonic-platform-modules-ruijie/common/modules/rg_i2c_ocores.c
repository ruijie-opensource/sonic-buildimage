...
#define SYMBOL_I2C_DEV_MODE   (1)   /* 通过I2C接口访问OCORE寄存器 */
#define FILE_MODE             (2)   /* 通过文件方式访问OCORE寄存器 */
#define SYMBOL_PCIE_DEV_MODE  (3)   /* 通过PCIe接口访问OCORE寄存器 */
#define SYMBOL_IO_DEV_MODE    (4)   /* 通过IO接口访问OCORE寄存器 */
 ...
 */
struct ocores_i2c {
    uint32_t base_addr;         /* ocore寄存器基址 */
    uint32_t reg_shift;
    uint32_t reg_io_width;
    unsigned long flags;        /* 1表示轮询, 0表示中断 */
    ...
    uint32_t ip_clock_khz;      /* 主时钟频率 */
    uint32_t bus_clock_khz;     /* I2C总线频率 */
    void (*setreg)(struct ocores_i2c *i2c, int reg, u32 value);
    u32 (*getreg)(struct ocores_i2c *i2c, int reg);
    const char *dev_name;       /* 设备节点名称 */
    uint32_t reg_access_mode;   /* ocore寄存器的访问 */
    uint32_t big_endian;        /* 1大端模式, 0小端模式 */
    uint32_t irq_offset;        /* 中断号偏移 */
    rg_pci_dev_t rg_pci_dev;
    struct device *dev;
};

int g_rg_ocores_i2c_debug = 0;
int g_rg_ocores_i2c_error = 0;
int g_rg_ocores_i2c_xfer = 0;

module_param(g_rg_ocores_i2c_debug, int, S_IRUGO | S_IWUSR);
module_param(g_rg_ocores_i2c_error, int, S_IRUGO | S_IWUSR);
module_param(g_rg_ocores_i2c_xfer, int, S_IRUGO | S_IWUSR);

#define OCORES_I2C_VERBOSE(fmt, args...) do {                                        \
    if (g_rg_ocores_i2c_debug) { \
        printk(KERN_INFO "[OCORES_I2C][VER][func:%s line:%d]\r\n"fmt, __func__, __LINE__, ## args); \
    } \
} while (0)

#define OCORES_I2C_ERROR(fmt, args...) do {                                        \
    if (g_rg_ocores_i2c_error) { \
        printk(KERN_ERR "[OCORES_I2C][ERR][func:%s line:%d]\r\n"fmt, __func__, __LINE__, ## args); \
    } \
} while (0)

#define OCORES_I2C_XFER(fmt, args...) do {                                        \
    if (g_rg_ocores_i2c_xfer) { \
...
static int ocores_i2c_file_read(const char *path, uint32_t pos, uint8_t *val, size_t size)
{
    ...
    ret = vfs_llseek(filp, tmp_pos, SEEK_SET);
    if (ret < 0) {
        OCORES_I2C_ERROR("read vfs_llseek failed, ret=%d\r\n", ret);
        set_fs(fs);
        goto exit;
    }

    ret = kernel_read(filp, val, size, &(filp->f_pos));
    set_fs(fs);
    if (ret < 0) {
        OCORES_I2C_ERROR("read kernel_read failed, ret=%d\r\n", ret);
        goto exit;
    }
...
static int ocores_i2c_file_write(const char *path, uint32_t pos, uint8_t *val, size_t size)
{
    ...
    ret = vfs_llseek(filp, tmp_pos, SEEK_SET);
    if (ret < 0) {
        OCORES_I2C_ERROR("write vfs_llseek failed, ret=%d\r\n", ret);
        set_fs(fs);
        goto exit;
    }
    ret = kernel_write(filp, val, size, &(filp->f_pos));
    set_fs(fs);
    if (ret < 0) {
        OCORES_I2C_ERROR("write kernel_write failed, ret=%d\r\n", ret);
        goto exit;
    }
    vfs_fsync(filp, 1);
    filp_close(filp, NULL);
...
static const struct of_device_id ocores_i2c_match[] = {
    {
        .compatible = "ruijie,rg-i2c-ocores",
...
static int fpga_ocores_i2c_get_irq(struct ocores_i2c *i2c)
{
    ...
    dev = i2c->dev;
    rg_pci_dev = &i2c->rg_pci_dev;
    if (dev->of_node) {
        ret = 0;
        ret += of_property_read_u32(dev->of_node, "pci_domain", &rg_pci_dev->domain);
        ret += of_property_read_u32(dev->of_node, "pci_bus", &rg_pci_dev->bus);
        ret += of_property_read_u32(dev->of_node, "pci_slot", &rg_pci_dev->slot);
        ret += of_property_read_u32(dev->of_node, "pci_fn", &rg_pci_dev->fn);
        ...
        OCORES_I2C_VERBOSE("pci_domain:0x%x, pci_bus:0x%x, pci_slot:0x%x, pci_fn:0x%x.\n",
            rg_pci_dev->domain, rg_pci_dev->bus, rg_pci_dev->slot, rg_pci_dev->fn);
        ...
        OCORES_I2C_ERROR("CONFIG_OF must be defined\n");
        ret = -ENXIO;
    }
    return ret;
}
