...
static int rg_wdt_register_device(rg_wdt_priv_t *priv)
{
    int ret;
...
static int rg_wdt_probe(struct platform_device *pdev)
{
    ...
    if (pdev->dev.of_node) {
        ...
        priv->sysfs_kernel = rg_wdt_device->sysfs_kernel;
    ...
    if (priv->sysfs_kernel == 1) {
        ret = rg_wdt_register_device(priv);
        if (ret < 0) {
