 ...
 * Original Author: sonic_rd@ruijie.com.cn 2020-09-21
 */
...
#define SPIOC_IDLE_LOW        BIT(1)    /* 0:空闲时SCK为高 1:空闲是SCK为低 */
...
#define SYMBOL_I2C_DEV_MODE   (1)   /* 通过I2C接口访问SPI OCORE寄存器 */
#define FILE_MODE             (2)   /* 通过文件方式访问SPI OCORE寄存器 */
#define SYMBOL_PCIE_DEV_MODE  (3)   /* 通过PCIe接口访问SPI OCORE寄存器 */
#define SYMBOL_IO_DEV_MODE    (4)   /* 通过IO接口访问SPI OCORE寄存器 */
...
struct spioc {
    ...
    unsigned int reamin_len;    /* spi传输剩余长度 */
    unsigned int cur_pos;       /* 当前SPI传输的位置 */
    unsigned int cur_len;       /* 当前SPI传输的长度 */
    const u8 *txp;
    u8 *rxp;
    u8 chip_select;             /* SPI设备的片选号 */
    ...
    uint32_t base_addr;         /* spi ocore 寄存器基址 */
    uint32_t reg_shift;
    uint32_t reg_io_width;
    uint32_t num_chipselect;    /* spi控制器支持的cs数 */
    uint32_t freq;              /* spi控制器主时钟频率 */
    uint32_t big_endian;        /* 1大端模式, 0小端模式 */
    struct device *dev;
    int transfer_busy_flag;     /* spi传输忙标志 */
};
...
static int spioc_get_clkdiv(struct spioc *spioc, u32 speed)
{
    ...
    /* fs = fw/((DIV+2)*2) */
    /* DIV值0~0e, 最大频率为主时钟频率的1/4, spi器件频率超过主时钟的1/4, 按照最大配置*/
    if (speed > (rate / 4)) {
        div = 0;
...
/* 设置片选
 * 这里仅保存片选值，不直接写寄存器，不然发送的时候，还要回读再写。
 **/
...
/*
 * 每个transfer都可以设置时钟频率，根据transfer->speed_hz来设置
 * 如果spi transfer未设置speed_hz则采用SPI DEVICE的max_speed_hz
 * 注意：SPI频率最大仅支持主时钟频率的1/4 fsclk = fwb_clk /((DIVIDER+2)*2)
 * DIVIDER值是0~e，最大1/4, 最小 1/32
 **/
static int spioc_setup_transfer(struct spi_device *spi, struct spi_transfer *transfer)
{
    ...
    if (transfer != NULL) {
        ...
        /* spi transfer未设置speed_hz, 采用 spi->max_speed_hz*/
        if (hz == 0) {
            hz = spi->max_speed_hz;
        }
    } else {
