...
/**
 * dfd_get_watchdog_id - 获取watchdog编号
 * @wdt_index: watchdog索引号

 * return: 成功：返回watchdog编号
 *       ：失败：返回负值
 */
...
ssize_t dfd_watchdog_get_status(char *buf, size_t count)
{
    ...
    key = DFD_CFG_KEY(DFD_CFG_ITEM_WATCHDOG_DEV, watchdog_id, RG_WDT_TYPE_ENABLE);
...
ssize_t dfd_watchdog_get_timeout(char *buf, size_t count)
{
    ...
    key = DFD_CFG_KEY(DFD_CFG_ITEM_WATCHDOG_DEV, watchdog_id, RG_WDT_TYPE_TIMEOUT);
    ret = dfd_info_get_int(key, &value, NULL);
    if (ret < 0) {
        DFD_WDT_DEBUG(DBG_ERROR, "get watchdog timeout status, key: 0x%x\n", key);
        return -DFD_RV_DEV_NOTSUPPORT;
    }
    ...
    if (coefficient != 0) {
        value = value * coefficient / 1000;
    }

    DFD_WDT_DEBUG(DBG_VERBOSE, "get watchdog timeout status ok, watchdog index:%u, timeout:0x%x.\n",
        watchdog_id, value);
    return (ssize_t)snprintf(buf, count, "%d\n", value);
}
...
ssize_t dfd_watchdog_set_reset(int value)
{
    ...
    watchdog_id = dfd_get_watchdog_id(0);
    key = DFD_CFG_KEY(DFD_CFG_ITEM_WATCHDOG_DEV, watchdog_id, RG_WDT_TYPE_RESET);
    ret = dfd_info_set_int(key, value);
    if (ret < 0) {
