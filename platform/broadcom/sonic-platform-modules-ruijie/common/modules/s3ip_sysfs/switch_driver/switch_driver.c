...
#include "./include/rg_fan_driver.h"
#include "./include/rg_eeprom_driver.h"
#include "./include/rg_cpld_driver.h"
#include "./include/rg_fpga_driver.h"
#include "./include/rg_led_driver.h"
#include "./include/rg_slot_driver.h"
#include "./include/rg_sensors_driver.h"
#include "./include/rg_psu_driver.h"
#include "./include/rg_sff_driver.h"
#include "./include/rg_watchdog_driver.h"
...
static ssize_t dfd_get_main_board_temp_high(unsigned int temp_index, char *buf, size_t count)
{
    ...
    ret = dfd_get_temp_info(RG_MAIN_DEV_MAINBOARD, RG_MINOR_DEV_NONE, temp_index, RG_SENSOR_HIGH,
              buf, count);
    return ret;
}
...
static ssize_t dfd_get_main_board_temp_low(unsigned int temp_index, char *buf, size_t count)
{
    ...
    ret = dfd_get_temp_info(RG_MAIN_DEV_MAINBOARD, RG_MINOR_DEV_NONE, temp_index, RG_SENSOR_LOW,
              buf, count);
    return ret;
}
...
static ssize_t dfd_get_fan_vendor(unsigned int fan_index, char *buf, size_t count)
{
    ...
    ret = dfd_get_fan_info(fan_index, DFD_DEV_INFO_TYPE_PART_NAME, buf, count);
    return ret;
}
...
static ssize_t dfd_get_psu_vendor(unsigned int psu_index, char *buf, size_t count)
{
    ...
    ret = dfd_get_psu_info(psu_index, DFD_DEV_INFO_TYPE_NAME, buf, count);
    return ret;
}

/* Similar to dfd_get_psu_date */
...
static ssize_t dfd_get_psu_status(unsigned int psu_index, char *buf, size_t count)
{
    ...
    /* Not present rerutn 0 */
    if ((int32_t)simple_strtol(buf, NULL, 10) == PSU_STATUS_ABSENT) {
        memset(buf, 0, count);
        return (ssize_t)snprintf(buf, count, "%d\n", STATUS_ABSENT);
    }
    /* Check output and alarm */
    output_status = dfd_get_psu_output_status(psu_index);
    if (output_status < 0) {
        return output_status;
    }
    alert_status = dfd_get_psu_alert_status(psu_index);
    if (alert_status < 0) {
        return output_status;
    }

    if( output_status != NORMAL || alert_status != NORMAL) {
        return (ssize_t)snprintf(buf, count, "%d\n", STATUS_NOT_OK);
    }
    return (ssize_t)snprintf(buf, count, "%d\n", STATUS_OK);
}
...
static ssize_t dfd_get_psu_fan_speed_max(unsigned int psu_index, char *buf, size_t count)
{
    ssize_t ret;
    ...
    ret = dfd_get_psu_info(psu_index, DFD_DEV_INFO_TYPE_SPEED_MAX, buf, count);
    return ret;
}

static ssize_t dfd_get_psu_fan_speed_min(unsigned int psu_index, char *buf, size_t count)
{
    ssize_t ret;
    ...
    ret = dfd_get_psu_info(psu_index, DFD_DEV_INFO_TYPE_SPEED_MIN, buf, count);
    return ret;
}

static ssize_t dfd_get_psu_out_vol_max(unsigned int psu_index, char *buf, size_t count)
{
    ssize_t ret;
    ...
    ret = dfd_get_psu_info(psu_index, DFD_DEV_INFO_TYPE_OUT_VOL_MAX, buf, count);
    return ret;
}

static ssize_t dfd_get_psu_out_vol_min(unsigned int psu_index, char *buf, size_t count)
{
    ssize_t ret;
    ...
    ret = dfd_get_psu_info(psu_index, DFD_DEV_INFO_TYPE_OUT_VOL_MIN, buf, count);
    return ret;
}
...
static ssize_t dfd_get_watchdog_timeout(char *buf, size_t count)
{
    ...
    ret = dfd_watchdog_get_timeout(buf, count);
    return ret;
}
...
{
    /* add vendor codes here */
    return dfd_watchdog_set_timeout(value);
}
...
static ssize_t dfd_get_slot_vendor(unsigned int slot_index, char *buf, size_t count)
{
    ...
    ret = dfd_get_slot_info(slot_index, DFD_DEV_INFO_TYPE_PART_NAME, buf, count);
    return ret;
}
...
static struct switch_drivers_s switch_drivers = {
    ...
    .get_psu_fan_speed_max = dfd_get_psu_fan_speed_max,
    .get_psu_fan_speed_min = dfd_get_psu_fan_speed_min,
    .get_psu_out_vol_max = dfd_get_psu_out_vol_max,
    .get_psu_out_vol_min = dfd_get_psu_out_vol_min,
    /* transceiver */
