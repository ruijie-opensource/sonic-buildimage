...
/* 风扇转速相关固定值类型 */
typedef enum rg_fan_threshold_e {
    FAN_SPEED_MIN   = 1,        /* 最小值 */
    FAN_SPEED_MAX   = 2,        /* 最大值 */
    FAN_SPEED_TOLERANCE = 3,    /* 公差（误差） */
    FAN_SPEED_TARGET_0 = 0x10,  /* PWM=0时额定速度配置的index */
    ...
    FAN_SPEED_TARGET_100 = 0x1a, /* PWM=100时额定速度配置的index */

} rg_fan_threshold_t;

/* fan_threshold_[阈值类型(高8bit)+主设备类型(低8bit)]_[子设备ID(高4bit)+前后马达id(低4bit)] */
...
/**
 * dfd_get_fan_eeprom_mode - 获取风扇E2类型
 * return: 0：TLV
 *         1：FRU
 *       : 负值 - 读取失败
 */
...
/**
 * dfd_get_fan_present_status - 获取风扇转动状态
 * @index: 风扇的编号,从1开始
 * @motor_index：马达编号,从1开始
 * return: 0：STALL
 *         1：ROLL
 *       : 负值 - 读取失败
 */
...
/**
 * dfd_get_fan_present_status - 获取风扇在位状态
 * @index: 风扇的编号,从1开始
 * return: 0：ABSENT
 *         1：PRESENT
 *       : 负值 - 读取失败
...
/**
 * dfd_fan_product_name_decode - 风扇名称转换
 * @psu_buf: 原始风扇名称
 * @buf_len: fan_buf长度
 * return: 成功：0
 *       ：失败：返回负值
...
int dfd_get_fan_speed_target(unsigned int fan_index, unsigned int motor_index, int *value)
{
    ...
    /* 获取当前PWM对应的额定速度 */
    key1 = DFD_GET_FAN_THRESHOLD_KEY1((pwm / 10 + FAN_SPEED_TARGET_0), RG_MAIN_DEV_FAN);
...
/**
 * dfd_get_fan_motor_speed_tolerance - 获取风扇转速公差
 * @fan_index
 * @motor_index
 * @value 转速公差值
 */
static int dfd_get_fan_motor_speed_tolerance(unsigned int fan_index, unsigned int motor_index, int *value)
{
    ...
    /* 获取风扇的误差率 */
    key1 = DFD_GET_FAN_THRESHOLD_KEY1(FAN_SPEED_TOLERANCE, RG_MAIN_DEV_FAN);
    key2 = DFD_GET_FAN_THRESHOLD_KEY2(fan_type, motor_index);
    key = DFD_CFG_KEY(DFD_CFG_ITEM_FAN_THRESHOLD, key1, key2);
...
/**
 * dfd_get_fan_direction - 获取风扇风道类型
 * @fan_index: 风扇偏移，从1开始
 * @value 0：F2B, 1：B2F
 * @returns: 0成功，负值：失败
 */
...
/**
 * dfd_get_fan_motor_speed_max - 获取风扇转速最大值
 * @fan_index
 * @motor_index
 * @value 风扇转速最大值
 * @returns: 0成功，负值：失败
...
static int dfd_get_fan_motor_speed_max(unsigned int fan_index, unsigned int motor_index, int *value)
{
    ...
    key1 = DFD_GET_FAN_THRESHOLD_KEY1(FAN_SPEED_MAX, RG_MAIN_DEV_FAN);
    key2 = DFD_GET_FAN_THRESHOLD_KEY2(fan_type, motor_index);
    key = DFD_CFG_KEY(DFD_CFG_ITEM_FAN_THRESHOLD, key1, key2);
...
/**
 * dfd_get_fan_motor_speed_min - 获取风扇转速最小值
 * @fan_index
 * @motor_index
 * @value 风扇转速最小值
 * @returns: 0成功，负值：失败
...
static int dfd_get_fan_motor_speed_min(unsigned int fan_index, unsigned int motor_index, int *value)
{
    ...
    key1 = DFD_GET_FAN_THRESHOLD_KEY1(FAN_SPEED_MIN, RG_MAIN_DEV_FAN);
    key2 = DFD_GET_FAN_THRESHOLD_KEY2(fan_type, motor_index);
    key = DFD_CFG_KEY(DFD_CFG_ITEM_FAN_THRESHOLD, key1, key2);
