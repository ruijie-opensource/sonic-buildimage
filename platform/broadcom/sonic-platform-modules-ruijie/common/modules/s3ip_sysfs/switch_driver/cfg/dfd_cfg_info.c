 ...
 *
 * dfd库数据信息提取模块，将数据归总成int、buf两种类型
 *
 */
...
/**
 * dfd_2key_info_get_buf - 获取buf型信息
 * @key: 配置项搜索关键字
 * @buf: 信息buf
 * @buf_len: buf长度，长度需要不小于info_ctrl->len
 * @pfun: 数据转换函数指针
...
int dfd_info_set_int(int key, int val)
{
    ...
    if (IS_INFO_FRMT_BIT(info_ctrl->frmt)) {
        ...
        /* 写入数据值转换 */
        byte_tmp = (uint8_t)(val & 0xff);           /* bit格式数据低8位有效 */
        byte_tmp <<= info_ctrl->bit_offset;         /* 数值移位至对应位 */
        if (info_ctrl->pola == INFO_POLA_NEGA) {    /* 负极性数据取反 */
            byte_tmp = ~byte_tmp;
        }
        ...
        /* num buf设置之前转化好数值，XXX 暂时没有多字节int数据设置需求 */
        write_bytes = 1;
