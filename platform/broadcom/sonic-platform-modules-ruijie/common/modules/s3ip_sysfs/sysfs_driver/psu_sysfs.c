...
static ssize_t psu_fan_speed_max_show(struct switch_obj *obj, struct switch_attribute *attr, char *buf)
{
    unsigned int psu_index;
    int ret;

    check_p(g_psu_drv);
    check_p(g_psu_drv->get_psu_fan_speed_max);

    psu_index = obj->index;
    PSU_DBG("psu index: %u\n", psu_index);
    ret = g_psu_drv->get_psu_fan_speed_max(psu_index, buf, PAGE_SIZE);
    if (ret < 0) {
        PSU_ERR("get psu%u fan speed max failed, ret: %d\n", psu_index, ret);
        return (ssize_t)snprintf(buf, PAGE_SIZE, "%s\n", SYSFS_DEV_ERROR);
    }
    return ret;
}

static ssize_t psu_fan_speed_min_show(struct switch_obj *obj, struct switch_attribute *attr, char *buf)
{
    unsigned int psu_index;
    int ret;

    check_p(g_psu_drv);
    check_p(g_psu_drv->get_psu_fan_speed_min);

    psu_index = obj->index;
    PSU_DBG("psu index: %u\n", psu_index);
    ret = g_psu_drv->get_psu_fan_speed_min(psu_index, buf, PAGE_SIZE);
    if (ret < 0) {
        PSU_ERR("get psu%u fan speed min failed, ret: %d\n", psu_index, ret);
        return (ssize_t)snprintf(buf, PAGE_SIZE, "%s\n", SYSFS_DEV_ERROR);
    }
    return ret;
}

static ssize_t psu_out_vol_max_show(struct switch_obj *obj, struct switch_attribute *attr, char *buf)
{
    unsigned int psu_index;
    int ret;

    check_p(g_psu_drv);
    check_p(g_psu_drv->get_psu_out_vol_max);

    psu_index = obj->index;
    PSU_DBG("psu index: %u\n", psu_index);
    ret = g_psu_drv->get_psu_out_vol_max(psu_index, buf, PAGE_SIZE);
    if (ret < 0) {
        PSU_ERR("get psu%u vol max failed, ret: %d\n", psu_index, ret);
        return (ssize_t)snprintf(buf, PAGE_SIZE, "%s\n", SYSFS_DEV_ERROR);
    }
    return ret;
}

static ssize_t psu_out_vol_min_show(struct switch_obj *obj, struct switch_attribute *attr, char *buf)
{
    unsigned int psu_index;
    int ret;

    check_p(g_psu_drv);
    check_p(g_psu_drv->get_psu_out_vol_min);

    psu_index = obj->index;
    PSU_DBG("psu index: %u\n", psu_index);
    ret = g_psu_drv->get_psu_out_vol_min(psu_index, buf, PAGE_SIZE);
    if (ret < 0) {
        PSU_ERR("get psu%u vol min failed, ret: %d\n", psu_index, ret);
        return (ssize_t)snprintf(buf, PAGE_SIZE, "%s\n", SYSFS_DEV_ERROR);
    }
    return ret;
}
...
static struct switch_attribute psu_fan_speed_max_attr = __ATTR(fan_speed_max, S_IRUGO, psu_fan_speed_max_show, NULL);
static struct switch_attribute psu_fan_speed_min_attr = __ATTR(fan_speed_min, S_IRUGO, psu_fan_speed_min_show, NULL);
static struct switch_attribute psu_out_vol_max_attr = __ATTR(out_vol_max, S_IRUGO, psu_out_vol_max_show, NULL);
static struct switch_attribute psu_out_vol_min_attr = __ATTR(out_vol_min, S_IRUGO, psu_out_vol_min_show, NULL);
...
static struct attribute *psu_attrs[] = {
    ...
    &psu_fan_speed_max_attr.attr,
    &psu_fan_speed_min_attr.attr,
    &psu_out_vol_max_attr.attr,
    &psu_out_vol_min_attr.attr,
    NULL,
};
