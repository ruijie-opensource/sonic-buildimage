...
#define RG_QSFP_TX_FAULT_OFFSET       (4)
#define RG_QSFPDD_TX_FAULT_OFFSET     (17*128 + 135)
#define RG_QSFP_TX_DISABLE_OFFSET     (86)
#define RG_QSFPDD_TX_DISABLE_OFFSET   (16*128 + 130)
#define RG_QSFP_RX_LOS_OFFSET         (3)
#define RG_QSFPDD_RX_LOS_OFFSET       (17*128 + 147)
#define RG_QSFP_LP_MODE_OFFSET        (93)
#define RG_QSFPDD_LP_MODE_OFFSET      (26)
...
static ssize_t eth_optoe_type_show(struct switch_obj *obj, struct switch_attribute *attr, char *buf)
{
    ...
    ret = g_sff_drv->get_eth_optoe_type(sff_index, &optoe_type);
    if(ret < 0) {
        SFF_ERR("get_eth_optoe_type error. sff index:%u, ret:%d\n", sff_index, ret);
        return (ssize_t)snprintf(buf, PAGE_SIZE, "%s\n", SYSFS_DEV_ERROR);
    }

    SFF_DBG("eth_optoe_type_show ok. sff index:%u, optoe_type:%d\n", sff_index, optoe_type);
...
static ssize_t eth_tx_fault_show(struct switch_obj *obj, struct switch_attribute *attr, char *buf)
{
    ...
    if (module_type[0] == 0x03) {
        ...
        if ((module_type[0] == 0x11) || (module_type[0] == 0x0D)) {
            ...
            offset = RG_QSFP_TX_FAULT_OFFSET;
            mask = 0xf;
        } else if ((module_type[0] == 0x18) || (module_type[0] == 0x1e) || (module_type[0] == 0x1b)) {
            SFF_DBG("get eth%u module type is QSFP-DD\n", eth_index);
            offset = RG_QSFPDD_TX_FAULT_OFFSET;
            mask = 0xff;
        } else {
                   ...
                   char *buf)
{
    ...
    if (module_type[0] == 0x03) {
        ...
        if ((module_type[0] == 0x11) || (module_type[0] == 0x0D)) {
            SFF_DBG("get eth%u module type is QSFP\n", eth_index);
            offset = RG_QSFP_TX_DISABLE_OFFSET;
            mask = 0xf;
        } else if ((module_type[0] == 0x18) || (module_type[0] == 0x1e) || (module_type[0] == 0x1b)) {
            SFF_DBG("get eth%u module type is QSFP-DD\n", eth_index);
            offset = RG_QSFPDD_TX_DISABLE_OFFSET;
            mask = 0xff;
        } else {
            SFF_ERR("eth%u module is unknown, module_type:%d\n", eth_index, module_type[0]);
            return (ssize_t)snprintf(buf, PAGE_SIZE, "%s\n", SYSFS_DEV_ERROR);
...
static ssize_t eth_rx_los_show(struct switch_obj *obj, struct switch_attribute *attr, char *buf)
{
    ...
    if (module_type[0] == 0x03) {
        ...
        if ((module_type[0] == 0x11) || (module_type[0] == 0x0D)) {
            SFF_DBG("get eth%u module type is QSFP\n", eth_index);
            offset = RG_QSFP_RX_LOS_OFFSET;
            mask = 0xf;
        } else if ((module_type[0] == 0x18) || (module_type[0] == 0x1e) || (module_type[0] == 0x1b)) {
            SFF_DBG("get eth%u module type is QSFP-DD\n", eth_index);
            offset = RG_QSFPDD_RX_LOS_OFFSET;
            mask = 0xff;
        } else {
            SFF_ERR("eth%u module is unknown, module_type:%d\n", eth_index, module_type[0]);
            return (ssize_t)snprintf(buf, PAGE_SIZE, "%s\n", SYSFS_DEV_ERROR);
...
static ssize_t eth_low_power_mode_show(struct switch_obj *obj, struct switch_attribute *attr, char *buf)
{
    ...
    if (module_type[0] == 0x03) {
        ...
        if ((module_type[0] == 0x11) || (module_type[0] == 0x0D)) {
            SFF_DBG("get eth%u module type is QSFP\n", eth_index);
            offset = RG_QSFP_LP_MODE_OFFSET;
            mask = 0x3;
        } else if ((module_type[0] == 0x18) || (module_type[0] == 0x1e) || (module_type[0] == 0x1b)) {
            SFF_DBG("get eth%u module type is QSFP-DD\n", eth_index);
            offset = RG_QSFPDD_LP_MODE_OFFSET;
            mask = 0x10;
        } else {
            SFF_ERR("eth%u module is unknown, module_type:%d\n", eth_index, module_type[0]);
            return (ssize_t)snprintf(buf, PAGE_SIZE, "%s\n", SYSFS_DEV_ERROR);
