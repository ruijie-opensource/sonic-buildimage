...
/*
 * rg_get_main_board_temp_high - Used to get the highimum threshold of temperature sensor
 * filled the value to buf, the value is integer with millidegree Celsius
 * @temp_index: start with 1
 * @buf: Data receiving buffer
 * @count: length of buf
 ...
 * otherwise it returns a negative value on failed.
 */
static ssize_t rg_get_main_board_temp_high(unsigned int temp_index, char *buf, size_t count)
{
    ssize_t ret;
...
/*
 * rg_get_main_board_temp_low - Used to get the lowimum threshold of temperature sensor
 * filled the value to buf, the value is integer with millidegree Celsius
 * @temp_index: start with 1
 * @buf: Data receiving buffer
 * @count: length of buf
 ...
 * otherwise it returns a negative value on failed.
 */
static ssize_t rg_get_main_board_temp_low(unsigned int temp_index, char *buf, size_t count)
{
    ssize_t ret;
...
static struct s3ip_sysfs_temp_sensor_drivers_s drivers = {
    ...
    .get_main_board_temp_high = rg_get_main_board_temp_high,
    .get_main_board_temp_low = rg_get_main_board_temp_low,
};
