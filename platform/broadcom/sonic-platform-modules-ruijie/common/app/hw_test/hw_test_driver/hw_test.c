...
/*
 * hw_test.c
 * Original Author : sonic_rd@ruijie.com.cn, 2020-10-15
 *
 * History
 *    v1.0    sonic_rd@ruijie.com.cn  2020-10-15  Initial version.
 */
...
static LIST_HEAD(mdio_dev_list);                     /* 所有的mdio设备通过链表进行管理 */
static LIST_HEAD(phydev_list);                       /* 所有的phy设备通过链表进行管理 */
static struct class *class_mdio_bus = NULL;          /* 用来暂存内核中mdio_bus_class全局量的地址 */
...
/* 遍历bus class，找到所有的mdio dev, 并把找到的mdio dev挂到链表上，进行管理。*/
...
/* 遍历mdio bus，找到所有的phydev, 并把找到的phydev挂到链表上，进行管理。*/
...
/* 读phy索引为phy_index的phydev的寄存器regnum。*/
...
/* 往phy索引为phy_index的phydev的寄存器regnum写寄存器值val。*/
