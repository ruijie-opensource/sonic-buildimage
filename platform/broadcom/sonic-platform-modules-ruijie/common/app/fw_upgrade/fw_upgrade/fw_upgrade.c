...
{
    write_ilpc2ahb_addr(flash_base_addr); /* 写flash地址 */
    enable_bytes(1);    /* flash操作命令字仅有1字节,需要将AHB配置成1字节模式 */
    write_addr_port(SUPERIO_REG7, LPC_ADDR_PORT);
    write_data_port(cmd & MASK, LPC_DATA_PORT); /* 写flash命令字 */
    trigger_ilpc2ahb_write();
    enable_bytes(4);
...
/* 根据WDT2状态寄存器的bit1判断, 0:主BMC启动, 1:从BMC启动 */
...
static void get_flash_base_and_ce_ctrl(int current_bmc, int cs, uint32_t *flash_base_addr, uint32_t *ce_ctrl_addr)
{
    ...
    /* 读Address Decoding Range Register获取flash的base addr */
...
static int write_bmc_flash_page(flash_info_t* info, uint32_t page_addr, uint8_t *p, int len)
{
    ...
    while (len) {
        write_bmc_flash_data((*(uint32_t *)(p + pos))); /* 每次写4个字节 */
        pos += 4;
        len -= 4;
    }
    pull_ce_up(info);
...
static int check_chip(uint32_t file_size, uint8_t *p, flash_info_t* info)
{
    ...
    /* 发送READ flash命令 */
    pull_ce_down(info);
    send_cmd(info->flash_base_addr, COMMON_FLASH_READ); /* flash读命令 */
    write_bmc_flash_addr(info->flash_base_addr); /* 读的flash起始地址 */
    while (1) {
...
static int read_single_bmc_flash(flash_info_t* info, uint32_t start_addr, int read_size, int is_print)
{
    ...
    pull_ce_down(info);
    send_cmd(info->flash_base_addr, COMMON_FLASH_READ); /* flash读命令 */
    write_bmc_flash_addr(flash_start_addr); /* 读的flash起始地址 */
    while (1) {
