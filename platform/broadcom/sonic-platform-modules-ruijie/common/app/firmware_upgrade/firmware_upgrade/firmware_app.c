...
static int firmware_cpld_test(int chain)
{
    int ret;
    int fd;
    char dev_file_name[FIRMWARE_NAME_LEN];
    ...
    memset(dev_file_name, 0, FIRMWARE_NAME_LEN);
    snprintf(dev_file_name, FIRMWARE_NAME_LEN - 1, "/dev/firmware_cpld_ispvme%d", chain);
    ...
    dbg_print(is_debug_on, "start to ispvme upgrade test.\n");
    ret = ispvme_test(fd);

    if (ret != FIRMWARE_SUCCESS) {
        dbg_print(is_debug_on, "Error: Failed to upgrade test ret=%d.\n", ret);
        close(fd);
        return ERR_FW_DO_CPLD_UPGRADE;
    }
    close(fd);
...
{
    int ret;
    int main_type, sub_type, slot, chain;
    ...
    if (argc == 5) {
        ...
        chain = strtoul(argv[3], NULL, 10);
        printf("+=====================================+\n");
        printf("|Begin to upgrade test, please wait...|\n");
        ret = firmware_cpld_test(chain);
        if (ret == FIRMWARE_SUCCESS) {
            printf("|       Upgrade test succeeded!       |\n");
            printf("+=====================================+\n");
            dbg_print(is_debug_on, "Sucess to upgrade test.\n");
            return FIRMWARE_SUCCESS;
        } else if (ret == ERR_FW_DO_UPGRADE_NOT_SUPPORT) {
            printf("|     Not support to upgrade test!    |\n");
            printf("+=====================================+\n");
            ...
            dbg_print(is_debug_on, "Failed to upgrade test (%d)\n", ret);
            printf("|         Upgrade test failed!        |\n");
            printf("+=====================================+\n");
            return ret;
        }
