...
#define DFD_I2C_RETRY_TIME              (50000 / DFD_I2C_RETRY_SLEEP_TIME)  /* 50ms次尝试 */
...
/**
 * dfd_read_port_i2c_one_time - 读i2c
 * @i2c_name: i2c设备名称
 * @dev_addr: 设备地址
 * @offset_addr: 偏移地址
 * @recv_buf: 读缓冲区
 * @size:  读buf的大小 
 * @addr_type: 0:单byte偏移访问，1:2byte偏移访问 
 *
...
/**
 * dfd_write_port_i2c_one_time - 写i2c
 * @i2c_name: i2c设备名称
 * @dev_addr: 设备地址
 * @offset_addr: 偏移地址
 * @write_buf: 写缓冲区
                                    ...
                                    uint8_t *write_buf, int32_t size,int addr_type)
{
    ...
    for (index = 0; index < size; index++) {
            ...
            usleep(5000);   /* SMBUS不加延时写多个字节会写入失败，这里延时5ms(经验值) */
    }
