...
class PortHsdkClass():
    '''PortHsdkClass类主要用于存储端口在不同层级的别名及相关信息'''
...
class PortHsdkUtil():
    ...
    class ReUtil():
        ...
        def getre_output_bcm_port(output):
            '''解析返回结果，获取bcm_port/mgmt_bcm_port'''
...
class PortHsdkTest():
    ...
    def __parse_port_config(self):
        '''解析port_config, 获取lport, interface, phy_port'''
    ...
    def __parse_yaml_data(self):
        '''解析yaml中port_id、phy_port_id行的配置'''
    ...
    def __get_global_bcmports(self):
        '''获取ps返回结果中的bcm_port, unit_port部分'''
    ...
    def get_prbs_port_range(self):
        '''获取面板口unit_port范围(不包含mgmt口和loopback口)'''
...
class PortHsdkKrTest(PortHsdkTest):
    ...
    def install_pktgen_mode(cls):
        '''查询当前系统是否加载了pktgen模块，如果没有则载入pktgen模块'''
...
class PortScnFn():
    ...
    def prbs_mac_sys_line_result(**kwargs):
        '''ExtPhy prbs mac、sys、line端开始测试, 并检测测试结果'''
    ...
    def sdk_cmd_redirect_judge(redirect):
        '''判断当前是否开启了输入重定向，再按照传入的redirect进行设置'''
