...
class Wedge():
    ...
    VERSION = 0x03                       # E2PROM文件定义的版本号，固定为0x03
    WEDGE_SIZE = 196                     # WEDGE EEPROM 有效数据长度196(包括最后一个字节的CRC8)
    ...
    def generate_value(self, size=256):
        ...
        # Local MAC Address
        status, mac = mac_addr_decode(self.fbw_local_mac)
        ...
        status, mac = mac_addr_decode(self.fbw_mac_base)
        if isValidMac(mac) is False:
    ...
    def decode(self, e2):
        ...
        if head != self.MAGIC_HEAD_INFO:
            raise WedgeException("Wedge eeprom head info error, not Wedge eeprom type, head:0x04x" % head, -10)
        self.magic = "0x%04x" % self.MAGIC_HEAD_INFO
        e2_index += self._FBW_EEPROM_F_MAGIC

        # E2版本号校验
        if ord(e2[e2_index]) != self.VERSION:
            raise WedgeException("Wedge eeprom version: 0x%02x, don't support" % ord(e2[e2_index]), -10)
        self.fbw_version = self.VERSION
        e2_index += self._FBW_EEPROM_F_VERSION

        # crc8校验
        crc8_calc = int(self.wedge_crc8(e2[0:self.WEDGE_SIZE-1]), 16)
        if crc8_calc != ord(e2[self.WEDGE_SIZE-1]):
            raise WedgeException("Wedge eeprom crc check error", -10)
        self.fbw_crc8 = crc8_calc
