...
def fac_fru_fan_setmac(bus,loc,value):
    ...
    if status == False:
        return False
    status = I2CUTIL.writeToFanE2File(bus, loc, value)
    if status == False:
        return False
    I2CUTIL.closeFanE2Protect()
    return True
...
def test_fan_fru_e2set(product_info,frulist):
    ...
    for fan in fans:
        try:
            ...
            # board拓展字段boardextra1作为硬件版本号，不去修改,从boardextra2开始
            for i in range(2, 11):
...
def generate_onie_tlv_value(product_info,is_sonic = True):
    ...
    try:
        ...
        onietlv = onie_tlv()
        rets    = onietlv.decode(eeprom)
        if onietlv.serialnum == "":     #没有semac过,只设置一个ID
            ...
            #用配置文件中的信息替换
            vendor_ext = product_info.get("RUIJIE_VENDOR_EXTENSION")
            if vendor_ext is None:
                _value[onietlv.TLV_CODE_VENDOR_EXT] = onietlv.generate_ext(product_info.get("CARDID"))  # 生成锐捷特有的id
            else:
            ...
            _value[onietlv.TLV_CODE_PRODUCT_NAME] = product_info.get("RUIJIE_PRODUCTNAME")
            _value[onietlv.TLV_CODE_PART_NUMBER]    = product_info.get("RUIJIE_PART_NUMBER")
            _value[onietlv.TLV_CODE_LABEL_REVISION] = product_info.get("RUIJIE_LABEL_REVISION")
            _value[onietlv.TLV_CODE_PLATFORM_NAME]  = product_info.get("RUIJIE_PLATFORM_NAME")
            _value[onietlv.TLV_CODE_ONIE_VERSION]   = product_info.get("RUIJIE_ONIE_VERSION")
            _value[onietlv.TLV_CODE_MAC_SIZE]       = product_info.get("RUIJIE_MAC_SIZE")
            _value[onietlv.TLV_CODE_MANUF_NAME]     = product_info.get("RUIJIE_MANUF_NAME")
            _value[onietlv.TLV_CODE_MANUF_COUNTRY]  = product_info.get("RUIJIE_MANUF_COUNTRY")
            _value[onietlv.TLV_CODE_VENDOR_NAME]    = product_info.get("RUIJIE_VENDOR_NAME")
            _value[onietlv.TLV_CODE_DIAG_VERSION]   = product_info.get("RUIJIE_DIAG_VERSION")
            _value[onietlv.TLV_CODE_SERVICE_TAG]    = product_info.get("RUIJIE_SERVICE_TAG")
...
def test_bmc_tlv_e2set(productname):
    ...
    try:
        ...
        #生成BMC ONIE-E2烧录文件
        RET = generate_onie_tlv_value(product_info,is_sonic = True) # 读X86端的ONIE-E2来生成烧片
        if RET[RETURN_KEY1] < 0:
...
def fac_init_cardidcheck():
    onie_tlv = onietlv()
    rest = getsyseeprombyId(onie_tlv.TLV_CODE_RJ_CARID)  # 判断cardId是否相同
    if rest == None:
        print("需要烧写bin文件")
        return False
    return True
    ...
    for slot in tlv_slots:
        try:
            ...
            if len(rets) == 0:
                ...
                continue
            # 根据配置文件及TLV子卡的序列号和硬件版本号生成FRU烧片文件
            slotfru = ipmifru()
