...
class RefreshUpgradeBase(object):
    ...
    def refresh_file_upgrade(self):
        try:
            ...
            # pre operation
            for init_cmd_config in self.init_cmd_list:
                ret, log = set_value(init_cmd_config)
                if ret == False:
                    warmupgradeerror(log)
                    return False, log
            warmupgradedebuglog("%s init cmd all set success" % self.device_name)
            ...
            if self.refresh_file is not None:
                ...
                if status:
                    log = "%s refresh file upg failed" % self.device_name
                    warmupgradeerror(log)
                    return False, log
            ...
            # end operation
            RET = 0
            for save_set_reg_config in self.save_set_reg_list:
                ret, log = self.recover_save_value(save_set_reg_config)
                if ret == False:
                    RET = -1
                    warmupgradeerror("%s recover save value failed" % self.device_name)
                    warmupgradeerror(log)

            for finish_cmd_config in self.finish_cmd_list:
                ret, log = set_value(finish_cmd_config)
                if ret == False:
                    RET = -1
                    warmupgradeerror("%s finish cmd exec failed" % self.device_name)
                    warmupgradeerror(log)
            if RET != 0:
                return False, "finish cmd exec failed"
            warmupgradedebuglog("%s finish cmd all set success" % self.device_name)
...
class RefreshUpgrade(RefreshUpgradeBase):
    ...
    def get_devtype(self):
        super(RefreshUpgrade, self).get_devtype()
        return self._devtype

    def get_subtype(self):
        super(RefreshUpgrade, self).get_subtype()
...
class WarmBasePlatform():
    ...
    def linecard_present_check(self, slot_name, slot_present_config):
        ...
        if (present_status == "ABSENT") or (present_status == "NOT OK"):
