...
class IntelligentMonitor():
    ...
    def debug_init(self):
        global debuglevel
        global INTELLIGENT_MONITOR_DEBUG_FILE
    ...
    def dcdc_whitelist_check(self, dcdc_name):
        try:
            ...
            if gettype is None or checkbit is None or okval is None:
                ...
                return False
            ret, value = get_value(check_item)
            if ret:
                retval = value
            else:
                monitor_syslog('%%INTELLIGENT_MONITOR-3-DCDC_WHITELIST_FAILED: %s check value error: %s' % (dcdc_name, value))
                return False
