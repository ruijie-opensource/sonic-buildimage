...
class BasePlatform():
    ...
    def recover_save_value(self, config):
        ...
        if val is None:
            ...
            if ret == False:
                upgradeerror(log)
                return False, "recover save value write failed"
    ...
    def subprocess_firmware_upgrade(self, config, file, main_type, sub_type, slot):
        try:
            ...
            for init_cmd_config in init_cmd_list:
                ...
                if ret == False:
                    return False, log
            upgradedebuglog("%s firmware init cmd all set success" % dev_name)
            ...
            if os.path.exists(UPGRADE_DEBUG_FILE):
                ...
                status, output = exec_os_cmd(command)
            if status:
                upgradeerror("%s firmware upgrade failed" % dev_name)
                return False, output
            ...
            for finish_cmd_config in finish_cmd_list:
                ...
                if ret == False:
                    upgradeerror("%s finish cmd exec failed" % dev_name)
                    upgradeerror(log)
                    RET = -1
            if RET != 0:
                return False, "%s firmware finish cmd exec failed" % dev_name
            upgradedebuglog("%s firmware finish cmd all set success" % dev_name)
    ...
    def subprocess_test_upgrade(self, config, file, main_type, sub_type, slot):
        try:
            ...
            for init_cmd_config in init_cmd_list:
                ...
                if ret == False:
                    return False, log
            upgradedebuglog("%s test init cmd all set success" % dev_name)
            time.sleep(0.5)  # delay 0.5s after execute init_cmd
            ...
            if os.path.exists(UPGRADE_DEBUG_FILE):
                ...
                status, output = exec_os_cmd(command)
            if status:
                upgradeerror("%s test upgrade failed" % dev_name)
                return False, output
            ...
            for finish_cmd_config in finish_cmd_list:
                ...
                if ret == False:
                    ...
                    RET = -1
            if RET != 0:
                return False, "%s finish cmd exec failed" % dev_name
            upgradedebuglog("%s test finish cmd all set success" % dev_name)
    ...
    def upgrade_fool_proofing(self, conf):
        ...
        try:
            ...
            if dev_model != self.VERSION:
                msg = "upgrade fool proofing failed, device model: %s, upgrade file version: %s" % (dev_model, self.VERSION)
                upgradedebuglog(msg)
                return False, msg
        ...
        try:
            ...
            for test_config in device_list:
                ...
                if pre_check_conf is not None:
                    ...
                    if status is False: # 检查失败，不对该链路进行测试
                        pre_check_failed += 1
    ...
    def do_bmc_upgrade_main(self, file, chip_select, erase_type):
        ...
        ret, log = self.upgrading(bmc_upgrade_config, file, self.devtype, self.subtype, chip_select, BMC_UPGRADE, erase_type)
        if ret == True:
...
class FwUpg(object):
    ...
    def do_file_cold_upg(self, fw_upg_instance):
        try:
            ...
            fw_upg_subype = fw_upg_instance.subtype
            fw_upg_slot = fw_upg_instance.slot
            ret, log = self.upg_platform.upgrading(fw_upg_config, fw_upg_file, fw_upg_devtype, fw_upg_subype, fw_upg_slot, COLD_UPGRADE)
            if ret is False:
    ...
    def do_file_init_check(self, file_path, slot, upg_type):
        ...
        if slot is None: # traverse all slots
            for i in range(0, self.max_slot_num + 1):
                ...
                if ret != CHECK_OK:
                    upgradedebuglog("file: %s, slot%d initial check not ok, ret: %d, msg: %s" % (file_path, i, ret, log))
                    if ret == ERR_FW_CARD_ABSENT or ret == ERR_FW_HEAD_CHECK or ret == ERR_FW_FOOL_PROOF:
