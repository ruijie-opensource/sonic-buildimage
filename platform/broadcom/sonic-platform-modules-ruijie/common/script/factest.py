...
def sync_bmc_tlv_e2(rst_arr):
    ...
    if tmp.has_key("BMC_access") and tmp.get("BMC_access") == 1: #BMC_access为1时，在BMC端同步做E2同步
...
def writeToEEprom(rst_arr):
    ...
    for item in rst_arr:
        rji2cset(E2_LOC["bus"], E2_LOC["devno"], index, ord(item))
        index += 1
    if TESTCASE.has_key('bmctlve2sync'):
...
def get_pmc_register(reg_name):
    ...
    try:
        if (not os.path.isfile(mb_reg_file)):
            print(mb_reg_file,  'not found !')
            return retval
        ret, retval = read_sysfs(mb_reg_file)
        if ret is not True:
           log_error("read_sysfs %s, retrun %s" % (mb_reg_file, retval)) 
           retval = 'ERR'
...
def check_yes_no(msg):
    ret = raw_input("{}[Yes/no]：".format(msg))
...
def get_mgmt_version():
    cmd = TESTCASE.get("get_mgmt_version_cmd", "ethtool -i eth0")
    valstr = ""
    ret , log = rj_os_system(cmd)
    if ret != 0:
        return False
...
def show_harddisk_info():
    ...
    if len(log):
        ...
        if ind is False:
            RET[RETURN_KEY2] = "smartctl cmd exec fail"
            RJPRINT("   %s" %(val))
            return RET
    else:
...
def get_cpld_version():
    ...
    for cpld in CPLDVERSIONS:
        ret, data = get_value(cpld)
        if ret is False:
            totalerr -= 1
            log_error("get_cpld_version faile, log:%s" % (data))
            result.append([ cpld.get('name',None),"NA","NA" ])
        else:
            result.append([ cpld.get('name',None),"%s" %  (data.replace("\x00", "").replace(" ", ""))])
...
def show_fpga_version():
    ...
    for fpga_show in TESTCASE.get("FPGA_INFO", None):
        fpga_name = fpga_show.get('fpga_name', None)
        if fpga_name is None:
            log_error("get fpga name failed.\n")
            return -1
        fpga_name = fpga_name.replace("\x00", "").strip()
        fpga_sub_check = fpga_check.get(fpga_name, None)
        RJPRINT("  %s:" % (fpga_name))
        for item in fpga_show.get('value'):
            ret, data = get_value(item)
            if ret is False:
                totalerr = -1
                log_error("fpga get %s faile, log:%s" % (item.get("name", "error para"), data))
                RJPRINT("\tget %s failed: fpga" % item.get("name", "error para"))
            else:
                data = "".join(data.split()[::-1])
                value_name = item.get("name", "error para").replace("\x00", "").strip()
                fpga_check_data = fpga_sub_check.get(value_name, None)
                if fpga_check_data is None:
                    #忽略比较
                    continue
                fpga_check_data = fpga_check_data.replace("\x00", "").strip()
                if fpga_check_data == data:
                    RJPRINT("       %-15s： %s" % (value_name, data.replace("\x00", "").replace(" ", "")))
                else:
                    totalerr = -1 
                    RJPRINT("       %-15s： 匹配失败, 设备版本:%s, 期望版本:%s" % 
                                (value_name, data.replace("\x00", "").replace(" ", ""), fpga_check_data))
    return totalerr
...
def check_cpu_mac():
    ...
    if ret != 0:
        ...
        ret, log2 = log_os_system(cmd, 0)
        if ret != 0:
            log_debug("\n get eth0 info failed")
            return {RETURN_KEY1: -1, RETURN_KEY2: "get eth0 info failed"}
    str_eth_tmp = re.subn('[\r\n\t]', ' ', log2.lower())
...
def show_device_mac():
    ...
    if FACTESTMODULE.get("bmc_diag", 0) == 1:
...
def test_sysinfo_part():
    ...
    if sdk_version is False:
        ...
        sdk_version_check = TESTCASE.get('dev_info').get('sdk_version', sdk_version)
        sdk_version_check = sdk_version_check.replace("\x00", "").strip()
        sdk_version = sdk_version.replace("\x00", "").strip()
        if sdk_version_check == sdk_version:
...
def get_onie_full_version():
    onie_version_check = TESTCASE.get('dev_info').get('onie_version', None).replace("\x00", "").strip()
    onie_build_date_check = TESTCASE.get('dev_info').get('onie_build_date', None).replace("\x00", "").strip()
    ...
    if onie_version is None or onie_build_date is None:
        log = "读失败.读出的 onie_version:%s, onie_build_date:%s" % (onie_version, onie_build_date)
        log_debug(log)
        return False, log
    if onie_version != onie_version_check :
        ...
        log_debug(log)
        return False, log
    if onie_build_date != onie_build_date_check :
        log = "onie_build_date校验失败.设备版本:%s, 期望版本:%s" % (onie_build_date, onie_build_date_check)
        log_debug(log)
        return False, log
    ...
    if onie_sub_version_check is not None:
        ...
        if onie_sub_version != onie_sub_version_check:
            log = "onie_sub_version校验失败.设备版本:%s, 期望版本:%s" % (onie_sub_version, onie_sub_version_check)
            log_debug(log)
            return False, log

    return True, "%s %s %s" % (onie_version, onie_build_date, onie_sub_version)
...
def get_bios_version():
    ...
    log = "      %-21s： %s\n      %-21s： %s\n      %-21s： %s\n" % ("vendor", vendor_check, "version", version_check, "release_date", release_check)
    return True, log
...
def test_sysinfo():
    ...
    for item in val:
        ...
        if cpld_check[item[0]] == "%s" % (item[1]):
            RJPRINT("  %-20s： %s" %(item[0],item[1]))
        else:
            RJPRINT("  %-20s： CPLD版本不匹配. 设备版本:%s, 期望版本:%s" %(item[0], item[1], cpld_check[item[0]]))
            totalerr -= 1
    ...
    if vals is not None :
        for item in vals:
            ...
            for i in range(len(chr_list) - 1):
                chr_str += chr(int(chr_list[i + 1],16))
...
def test_data(item, data):
    ...
    if ret is False:
        log_error("set item:%s failed. log:%s" % (item, log))
        return ret, log
    ret, value = get_value(item)
    if ret is False:
        log_error("get item:%s failed. log:%s" % (item, log))
        return ret, value
    if data != value:
        return False, ("not equal: set data[0x%x] get value[0x%x]" % (data, value))
    return True, value
...
def test_cpld_new():
    ...
    if type(vals) == list:
        for item in vals:
            try:
                ret, data = test_data(item, 0x55)
                if ret is True:
                    RJPRINT("%-30s  0x%02x  %s" % (item.get('name'), data, 'OK'))
                else:
                    totalerr -= 1
                    ermsg = "%s %-30s  test value 0x55 failed.\n" % (ermsg, item.get('name'))

                ret, data = test_data(item, 0xaa)
                if ret is True:
                    RJPRINT("%-30s  0x%02x  %s" % (item.get('name'), data, 'OK'))
                else:
                    totalerr -= 1
                    ermsg = "%s %-30s  test value 0xaa failed.\n" % (ermsg, item.get('name'))
            except Exception as e:
                log_debug(str(e))
...
def test_tempinfo_new():
    ...
    for i in range(len(L)):
        ...
        with open(valloc, 'r') as fd2:
            ...
            if rval >= 0:
                RJPRINT("  %-25s : %d C" %( retval2.strip(), rval))
            else:
                RJPRINT("  %s: 读出温度为负值，温度异常" % retval2.strip())
                totalerr -= 1
        #print retval3
        ...
        try:
            ...
            if fru.productInfoArea is not None:
                ...
                if psuModleName_Dict is not None:
                    for psu_name in psuModleName_Dict.keys():
...
def test_power_status():
    ...
    for item_psu in psu_conf:
        retval = None
        statusbit = item_psu.get('statusbit')
        presentbit = item_psu.get('presentbit')
        alertbit = item_psu.get('alertbit')
        ret, retval = get_value(item_psu)
        if ret is False:
            totalerr -=1
            errmsg = " %s  %s" % (errmsg, "读取失败")
            log_error("%s get value false, log:%s" %(item_psu.get('name'), retval))
            continue
        ...
        msg = "%s \n" \
              "    在位状态: %s\n"\
              "    输出状态: %s \n"\
              "    告警状态: %s \n"% (item_psu.get('name'),psupresent.get(val_t),psuoutput.get(val_status),psualert.get(val_alert))
        RJPRINT(msg)
...
def test_fan_speed():
    ...
    low_speed = fanlevel.get('low_speed')[0] if fanlevel.has_key('low_speed') else None
    high_speed = fanlevel.get('high_speed')[2] if fanlevel.has_key('high_speed') else None
    for item_fan in fanloc:
        RJPRINT(item_fan.get('name'))
        for fanstatus in item_fan["childfans"]:
            if fanstatus.has_key('low_speed'):
...
def set_fans_ratio(ratio):
    ...
    for i in range(fansnum):
        ...
        for i in range(morotsnum):
            name = "motor%d" % i
            url = "/sys/rg_plat/fan/%s/%s/ratio" % (fanname, name)
    ...
    try:
        ...
        for i in range(fansnum):
            ...
            for i in range(morotsnum):
                ...
                morot_dit = fan_dit.get(name)
                nowspeed = morot_dit.get("speed")
                if nowspeed not in range(morotlow, morothigh):
                    totalerr -= 1
...
def startFanctrol():
    if STARTMODULE['fancontrol'] == 1:
        ...
        if len(rets) == 0:
            os.system(cmd)
    if STARTMODULE.has_key("hal_fanctrl") and STARTMODULE['hal_fanctrl'] ==1:
...
def test_mgmt():
    ...
    if ret != True:
        RET = {RETURN_KEY1 : -1, RETURN_KEY2 : "Fail"}
        return RET
    if FACTESTMODULE.has_key("mgmttest") and FACTESTMODULE['mgmttest'] == 1:
...
def test_cpld_gpio():
    ...
    if len(log):
        ret, log = log_os_system("upgrade.py test cpld 0", 0)
        RJPRINT(log)
...
def fac_fans_show_fru(fans):
    ...
    for fan in fans:
        try:
            ...
            ret, eeprom = read_sysfs(I2CUTIL.getE2File(fan.get('bus'), fan.get('loc'))) # master-dts分支
            if ret is False:
                log_error("   Get %s eeprom failed. log:%s" %(fan.get('name'), eeprom))
                RJPRINT("读取风扇信息失败")
                continue
            if isinstance(eeprom, bytes):
                eeprom = typeTostr(eeprom)
            fru = ipmifru()
            fru.decodeBin(eeprom)
            ...
            totalerr -= 1
            log_error("   Get %s info failed. Exception:%s" %(fan.get('name'), e))
            RJPRINT("读取风扇信息异常")
    if totalerr <0:
        return False
    return True
...
def test_opt_module_online_e2_read():
    RET = {RETURN_KEY1 : 0,  RETURN_KEY2 : ""}
    present_list = []
    
    ret, log = log_os_system("show interfaces transceiver presence", 0)
    if "Traceback" in log or len(log) == 0 or "Unknown" in log or "FAILED" in log:
        RJPRINT( "在位检测失败，请确认光模块是否在位")
        RET[RETURN_KEY1] = -1
        RET[RETURN_KEY2] = log
        return RET
    ...
    for dev in dev_list:
        port_dict_info = {}
        port_dict_info.update({"port_num": i})
        cur_dev_list = dev.strip().split(" ", 1)
        if "Present" in dev:
            port_dict_info.update({"port_alias": cur_dev_list[0].strip()})
            present_list.append(port_dict_info)
        elif "Not present" in dev:
            log_debug("%03d is not present" % (i))
        else:
            log_error(dev)
            continue
        i += 1
    ...
    cmd = "show interfaces transceiver eeprom "
    totalerr = 0
    for port_dev in present_list:
        ret, log = log_os_system(cmd + port_dev["port_alias"], 0)
        if ret or "FAILED" in log:
            totalerr -= 1
            log_error("get port %d error. log:%s" %((port_dev["port_num"]), log))
            RJPRINT("%03d:  获取光模块信息失败" %(port_dev["port_num"]))
            continue
        ...
        for i in range(len(dev_list1)):
            ret = ''.join(re.findall('Vendor.*Name:.*', dev_list1[i]))
            if ret:
                ret = re.sub("Vendor.*Name:","", ret)
                port_dev.update({"Name": ret.replace(" ","")})

            ret = ''.join(re.findall('Vendor.*PN:.*', dev_list1[i]))
            if ret:
                ret = re.sub("Vendor.*PN:","", ret)
                port_dev.update({"PN": ret.replace(" ","")})
            else:
                ret = ''.join(re.findall('Part Number:.*', dev_list1[i]))
                if ret:
                    ret = re.sub("Part Number:","", ret)
                    port_dev.update({"PN": ret.replace(" ","")})
            
            ret = ''.join(re.findall('Vendor.*SN:.*', dev_list1[i]))
            if ret:
                ret = re.sub("Vendor.*SN:","", ret)
                port_dev.update({"SN": ret.replace(" ","")})
            else:
                ret = ''.join(re.findall('Serial Number:.*', dev_list1[i]))
                if ret:
                    ret = re.sub("Serial Number:","", ret)
                    port_dev.update({"SN": ret.replace(" ","")})
        RJPRINT("%03d:  PN:%-20s    SN:%s" %(port_dev["port_num"], port_dev["PN"], port_dev["SN"]))
...
def open_smi_access(whichone = "x86"):
    ...
    if whichone == "x86":
        opencmd = commands.get("CPU").get("open")
        ret, val = send_commands(opencmd)
        if FACTESTMODULE.get("bmc_diag", 0) == 1:
            closecmd = commands.get("BMC").get("close")
            ret1, val1 = send_commands(closecmd, False)
        else:
            ret1 = True
    elif whichone == "bmc":
        opencmd = commands.get("BMC").get("open")
        ret, val = send_commands(opencmd, False)
        closecmd = commands.get("CPU").get("close")
...
def factest_check(argv):
    ...
    if len(argv) == 2 and argv[1] == 'diag':
    ...
    elif len(argv) == 2 and argv[1] == 'diagusb':
        DIAGTEST = True
        ...
        # 执行巡检并保存数据到指定路径
        ret = test_all_check(save_path, diagtestall)
        ...
        if ret[RETURN_KEY1] == 0 and len(ret[RETURN_KEY2]) != 0:
            try:
                ...
                ret, log = log_os_system("mv " + tmp_file + " " + save_file, 0)
            except Exception as e:
                RJPRINT("巡检结果写入指定文件失败")
            RJPRINT("巡检执行完毕")
        else:
            RJPRINT("巡检执行失败")
        ...
        RJPRINT("巡检执行指令有误，请执行\"factest.py diagusb\"")
        #pass
