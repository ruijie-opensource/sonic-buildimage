...
def fru_eeprom_show(eeprom):
    try:
        fru = ipmifru()
        fru.decodeBin(eeprom)
        print("=================board=================")
        print(fru.boardInfoArea)
        print("=================product=================")
        print(fru.productInfoArea)
    except Exception as e:
        print(str(e))
    return
...
def eeprom_parase(eeprom_conf):
    ...
    if e2_type == "onie_tlv":
        onie_eeprom_show(binval)
    elif e2_type == "fru":
        fru_eeprom_show(binval)
    elif e2_type == "rg_tlv":
        rg_tlv_eeprom_show(binval)
    else:
...
# rg_tlv eeprom info decode
@main.command()
@click.argument('e2_path', required=True)
@click.argument('e2_size', required=False, default="256")
def rg_tlv(e2_path, e2_size):
    '''e2_path'''
    decode_eeprom_info("rg_tlv", e2_path, e2_size)
