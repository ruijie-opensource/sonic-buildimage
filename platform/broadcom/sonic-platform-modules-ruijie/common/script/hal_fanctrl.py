...
class fancontrol(object):
    ...
    def __init__(self):
        ...
        self.__check_crit_reboot_cmd = self.__fancontrol_para.get("check_crit_reboot_cmd", "/sbin/reboot")
    ...
    # 判断监控的温度点中，是否有温度点超过了 critical 阈值(3次去抖)，返回True/False
    def is_temp_critical(self):
        critical_flag = False
        # 遍历所有监控的温度点
        for temp_threshold in self.__temps_threshold_config.values():
            temp_threshold['critical_flag'] = False # 默认设置该温度点的critical_flag为False
            if temp_threshold['ignore_threshold']:
                continue
            if temp_threshold['critical_num'] >= self.__temp_critical_num: # 判断是否连续3次超过 critical 阈值
                critical_flag = True
                temp_threshold['critical_flag'] = True # 设置该温度点的critical_flag为True
    ...
    # 判断是否有温度点达到 critical 告警阈值, 返回True/False
    def checkTempCritical(self):
        try:
            if self.is_temp_critical(): # 某个温度点达到critical阈值，进行countdown
                self.__critical_countdown = self.__temp_critical_countdown
                fancontrol_debug("temp is over critical")
                return True
            else: # 所有温度点均未达到critical阈值，countdown -= 1
    ...
    # 判断监控的温度点中，是否有温度点超过了 emergency 阈值(3次去抖)，返回True/False
    def is_temp_emergency(self):
        emergency_flag = False
        # 遍历所有监控的温度点
        for temp_threshold in self.__temps_threshold_config.values():
            temp_threshold['emergency_flag'] = False # 默认设置该温度点的emergency_flag为False
            if temp_threshold['ignore_threshold']:
                continue
            if temp_threshold['emergency_num'] >= self.__temp_emergency_num: # 判断是否连续3次超过 emergency 阈值
                emergency_flag = True
                temp_threshold['emergency_flag'] = True # 设置该温度点的emergency_flag为True
    ...
    # 判断是否有温度点达到 emergency 告警阈值, 返回True/False
    def checkTempEmergency(self):
        try:
            if self.is_temp_emergency(): # 某个温度点达到emergency阈值，进行countdown
    ...
    # 判断是否达到critical过温重启的条件(MAC温度超过critical阈值，或者其余温度均超过critical阈值)
    def checkTempRebootCrit(self):
        try:
            if self.is_temp_critical():
                temp_dict = dict(self.__temps_threshold_config)
                tmp = temp_dict.get(SWITCH_TEMP) # 判断是否MAC温度critical
    ...
    # 判断是否触发过温重启(连续3次checkTempRebootCrit均满足)
    ...
    # 判断是否触发过温重启(连续3次checkEmergReboot均满足)
    ...
    def abnormal_check(self):
        ...
        if self.__check_temp_emergency == 1:
            status = self.checkTempEmergency() # 判断是否有温度点达到 emergency 告警阈值
            if status is True:
                ...
                if self.__check_emerg_reboot_flag == 1:
                    self.checkEmergReboot()  # 判断是否触发过温重启
            else: # 温度未达到Emergency，判断Countdown是否为0
        ...
        if self.__check_temp_critical == 1:
            status = self.checkTempCritical() # 判断是否有温度点达到 critical 告警阈值
            if status == True:
                ...
                if self.__check_crit_reboot_flag == 1:
                    self.checkCritReboot()  # 判断是否触发过温重启
            else: # 温度未达到Critical，判断Countdown是否为0
    ...
    def check_fan_air_flow(self):
        ...
        if self.fan_air_flow_monitor:
            ...
            # 未开启风扇风道类型监控功能，风道不一致标志位设置为False
