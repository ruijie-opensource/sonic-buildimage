...
status_device_i2c_log="systemctl status rg_platform_driver.service"
status_device_i2c=`$status_device_i2c_log | grep "Active:" | grep "active (exited)"`
if [ -z "$status_device_i2c" ];then
    echo "rg_platform_driver service started fail"
    logger -t $ENV_CHECK_SH "`$status_device_i2c_log`"
else
    echo "rg_platform_driver service exited normally"
fi
