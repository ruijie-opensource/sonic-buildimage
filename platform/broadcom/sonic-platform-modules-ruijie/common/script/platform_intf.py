...
class IntfPlatform:
    ...
    ###########################################
    # reg_read - 读硬件寄存器
    # @dev_type: 寄存器类型，CPLD/FPGA等(0表示CPLD，1表示FPGA，暂无其他)
    # @dev_id: 器件ID(CPLD/FPGA可能会有多片)
    # @offset: 偏移地址
    # @size: 需要读取的数据大小
    # return:
    # @ret：True表示成功，False表示失败
    # @info：成功时返回内容数据是一个数据列表(元素是1个字节的int数据)。失败时返回为具体的失败信息
    ###########################################
    ...
    ###########################################
    # platform_reg_write - 写硬件寄存器
    # @dev_type: 寄存器类型，CPLD/FPGA等(0表示CPLD，1表示FPGA，暂无其他)
    # @dev_id: 器件ID(CPLD/FPGA可能会有多片)
    # @offset: 偏移地址
    # @val_list: 需要写入的数据列表(元素是1个字节的int数据)
    # return:
    # @ret：True表示成功，False表示失败
    # @info：成功时返回写入的长度。失败时返回为具体的失败信息
    ###########################################
    ...
    ###########################################
    # set_optoe_type - 设置端口对应的optoe驱动类型
    # @port: 面板端口号
    # @optoe_type: OPTOE类型
    # 1: OPTOE1
    # 2: OPTOE2
    ...
    ###########################################
    # get_optoe_type - 获取端口对应的optoe驱动类型
    # @port: 端口编号,从1开始
    # return:
    #  @ret：True表示成功，False表示失败
    #  @info：成功时为OPTOE类型。失败时返回为具体的失败信息
    # 1: OPTOE1
    # 2: OPTOE2
    # 3: OPTOE3
    ###########################################
...
###########################################
# platform_set_optoe_type - 设置端口对应的optoe驱动类型
# @port: 面板端口号
# @optoe_type: OPTOE类型
# 1: OPTOE1
# 2: OPTOE2
...
###########################################
# platform_get_optoe_type - 获取端口对应的optoe驱动类型
# @port: 端口编号,从1开始
# return:
#  @ret：True表示成功，False表示失败
#  @info：成功时为OPTOE类型。失败时返回为具体的失败信息
