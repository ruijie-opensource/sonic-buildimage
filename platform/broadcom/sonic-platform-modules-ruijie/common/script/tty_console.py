...
import os
import shutil
from ruijieutil import CompressedRotatingFileHandler
...
fh = CompressedRotatingFileHandler(console_logfile, mode='a', maxBytes=MAX_LOG_BYTES, backupCount=BACKUP_COUNT, encoding=None, delay=0)
fh.setLevel(logging.DEBUG)

# 设置日志格式
formatter = logging.Formatter("%(asctime)s - %(levelname)s - %(message)s")
fh.setFormatter(formatter)
#将相应的handler添加在logger对象中logger.addHandler(fh)
