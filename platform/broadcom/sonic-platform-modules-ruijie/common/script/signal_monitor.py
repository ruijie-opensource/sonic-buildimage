...
def timestr_convert_to_unix_ts(timestr):
    # ms时间转换为Unix时间戳
    # timestr ms时间,如 2020-01-01 00:00:00.000
    try:
...
class SignalRecord(object):
    ...
    def __init__(self, config):
        self.__dev_name = config["name"]                          # 信号记录设备名称
        self.__config_dev = config["config_dev"]                  # 配置 逻辑器件
        self.__record_dev = config["record_dev"]                  # 记录 逻辑器件
        self.__record_file_path = config["record_file"]           # 软件记录条数存放文件路径
        self.__signal_conf = config["signal_conf"]                # 信号配置列表
        self.__per_record_len = config.get("per_record_len", 16)  # 每条记录的长度
        self.__max_record_num = config.get("max_record_num", 512)  # 最大支持的记录条数
        # 配置 寄存器参数
        self.__unix_ts_set_reg = config.get("unix_ts_set_reg", 0xf1)  # Unix时间戳设置寄存器
        self.__unix_ts_len = config.get("unix_ts_len", 5)            # unix时间戳长度 5字节(40bit)
        self.__time_set_reg = config.get("time_set_reg", 0xf6)       # Unix时间戳配置生效寄存器
        self.__ufm_op_reg = config.get("ufm_op_reg", 0xf7)           # UFM操作寄存器
        self.__record_cnt_reg = config.get("record_cnt_reg", 0xf8)   # 计数寄存器
        # 记录 寄存器参数
        self.__record_mask_reg = config.get("record_mask_reg", 0x00)   # 标志位寄存器
        self.__record_flag = config.get("record_flag", 0xa5)           # 异常记录标志位, 已写为0XA5, 未写为0
        self.__record_ts_reg = config.get("record_ts_reg", 0x01)       # 记录时间戳
        self.__time_base = config.get("time_base", "2020-01-01 00:00:00.000")
        self.__earse_threshold = config.get("earse_threshold", 500)
        self.__update_unix_ts_cnt = config.get("update_unix_ts_cnt", 60)  # 默认轮询60次更新UNIX时间戳
        self.__unix_ts_base = 0    # base时间对应的unix时间戳
    ...
    def ufm_op_status(self):
        # 读UFM状态0: 空闲 1:loading/earsing, 失败-1
    ...
    def record_start_flag(self):
        # 判断是否已开启UFM关键信号记录功能 1:已开启 0: 未开启 -1:读失败
        status, value = dev_file_read(self.config_dev, self.ufm_op_reg, 1)
        if status is True:
    ...
    def __get_record_mask(self, record_num):
        # 读记录标志位 1:有记录 0:无记录 -1:读失败
        # @record_num 从1开始，最大为512
    ...
    def __key_signal_record(self, record_num):
        ...
        for item in self.signal_conf:
            ...
            for signal in signal_list:
                bit = signal["bit"]
                okval = signal["okval"]
                if ((record_value >> bit) & 0x01) != okval:  # 记录到异常信号
                    signal_syslog_info(
    ...
    def key_signal_check(self):
        ...
        if file_record_cnt < dev_record_cnt:
            ...
            self.file_record_cnt = dev_record_cnt  # 更新记录值

        # 判断是否执行UFM擦除操作
        if dev_record_cnt >= self.earse_threshold:
            if self.dev_record_cnt == dev_record_cnt:  # 无新纪录产生, 擦除UFM
    ...
    def cold_reboot_dev_init(self):
        ...
        # ufm_load
        # 读文件判断软件记录了多少条
        # 如果文件不存在:可能是升级了系统，冷重启
        #     从1~512遍历记录 -> 将所有记录写入syslog
        # 如果文件存在（根据文件值N）, record_num从N+1开始遍历 -> max
        # 设置UNIX时间戳，生效
        # 写文件值为0
        # 擦除UFM，启动CPLD开始记录
        # 初始化OK, 返回0, 初始化异常返回-1
        ...
        try:
            ...
            time.sleep(0.01)
            file_record_cnt = self.file_record_cnt
            if file_record_cnt < 0:  # 读文件失败
                file_record_cnt = 0  # 从头开始遍历记录
    ...
    def warm_reboot_dev_init(self):
        # 热启动初始化
        # 读CPLD状态，判断CPLD是否已开启关键信号记录
        # 未开启 >>> 第一次启用该功能
        #     1. 设置UNIX时间戳，生效; 写文件值为0; 擦除UFM，启动CPLD开始记录
        # 已开启
        #     1. 文件不存在（升级了sonic）>>> 将文件计数值设置为0
        # 初始化OK, 返回0, 初始化异常返回-1
        signal_syslog_info("%%SIGNAL_MONITOR-6-INFO: %s warm reboot init start." % self.dev_name)
        try:
            record_start_flag = self.record_start_flag

            if record_start_flag == -1:  # 读失败, 不确定之前的状态，按照冷启动处理
            ...
            if record_start_flag == 0:  # 未开启CPLD关键信号记录功能
                status, msg = self.set_device_timestamp()
                # 设置时间戳失败 退出
                if status is False:
                    signal_syslog_error(
...
def start():
    '''start signal record'''
    # 判断signal_monitor进程是否存在，限制只能跑一个
    ApplicationInstance()
