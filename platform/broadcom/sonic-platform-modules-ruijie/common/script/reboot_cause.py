...
class RebootCause():
    ...
    def debug_init(self):
        global debuglevel
        global REBOOT_CAUSE_DEBUG_FILE
        ...
        try:
            for item in item_list:
                ...
                if record_type == 'file':
                    ...
                    status, ret_t = rj_os_system(create_dir)
                    if status != 0:
                        RET["RETURN_KEY1"] = -1
                        record_syslog('%%REBOOT_CAUSE-3-EXCEPTION: create %s failed.' % os.path.dirname(file_path))
                        continue

                    status, date = rj_os_system("date")
                    ...
                    status, ret_t = rj_os_system(reocrd_cmd)
                    if status != 0:
                        RET["RETURN_KEY1"] = -1
                        record_syslog('%%REBOOT_CAUSE-3-EXCEPTION: get date failed.')
                        continue
    ...
    def run(self):
        try:
            ...
            rj_os_system("touch %s" % REBOOT_CAUSE_STARTED_FLAG)
            rj_os_system("sync")
            time.sleep(5)
            exit(0)
        except Exception as e:
