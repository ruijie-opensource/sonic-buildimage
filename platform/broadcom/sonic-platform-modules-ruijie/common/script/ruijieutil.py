...
def rji2cset_pec(bus, devno, address, byte):
...
def rji2cset_wordpec(bus, devno, address, byte):
...
def getsysmeminfo():
    ...
    if ret != 0 or len(log) <= 0:
        ...
        return False, error
    cmd = log + r"|grep -P -A5 \"Memory\s+Device\"|grep Size|grep -v Range"
    # 先获取总数
    result = []
