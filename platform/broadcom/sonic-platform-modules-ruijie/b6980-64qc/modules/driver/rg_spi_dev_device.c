...
#include <rg_spi_dev.h>

#define SPI_DEVICE_MAX_NUM (64)

static int g_rg_spi_dev_device_debug = 0;
static int g_rg_spi_dev_device_error = 0;

module_param(g_rg_spi_dev_device_debug, int, S_IRUGO | S_IWUSR);
module_param(g_rg_spi_dev_device_error, int, S_IRUGO | S_IWUSR);

#define RG_SPI_DEV_DEVICE_DEBUG_VERBOSE(fmt, args...) do {                                        \
    if (g_rg_spi_dev_device_debug) { \
        printk(KERN_INFO "[RG_SPI_DEV_DEVICE][VER][func:%s line:%d]\r\n"fmt, __func__, __LINE__, ## args); \
    } \
} while (0)

#define RG_SPI_DEV_DEVICE_DEBUG_ERROR(fmt, args...) do {                                        \
    if (g_rg_spi_dev_device_error) { \
        printk(KERN_ERR "[RG_SPI_DEV_DEVICE][ERR][func:%s line:%d]\r\n"fmt, __func__, __LINE__, ## args); \
    } \
} while (0)
...
static int __init rg_spi_dev_device_init(void)
{
    int i;
    ...
    RG_SPI_DEV_DEVICE_DEBUG_VERBOSE("enter!\n");
    ...
    for (i = 0; i < ARRAY_SIZE(spi_dev_device_info); i++) {
        master = spi_busnum_to_master(spi_dev_device_info[i].bus_num); /* 根据spi bus号获取控制器 */
        if (!master) {
...
static void __exit rg_spi_dev_device_exit(void)
{
    int i;
...
module_init(rg_spi_dev_device_init);
module_exit(rg_spi_dev_device_exit);
MODULE_DESCRIPTION("RG SPI DEV Devices");
MODULE_LICENSE("GPL");
