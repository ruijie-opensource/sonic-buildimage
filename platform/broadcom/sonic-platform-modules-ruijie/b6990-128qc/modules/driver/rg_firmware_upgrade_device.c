 ...
 * Original Author: sonic_rd@ruijie.com.cn 2021-03-17
 *
 * ko for firmware device
 * History
 *  [Version]        [Author]                   [Date]            [Description]
 *    v1.0      sonic_rd@ruijie.com.cn        2021-05-07          Initial version
 */
...
#include <rg_firmware_upgrade.h>

#define GPIO_D1700_OFFSET  (275)

static int g_rg_firmware_upgrade_debug = 0;
static int g_rg_firmware_upgrade_error = 0;

module_param(g_rg_firmware_upgrade_debug, int, S_IRUGO | S_IWUSR);
module_param(g_rg_firmware_upgrade_error, int, S_IRUGO | S_IWUSR);

#define RG_FIRMWARE_UPGRADE_DEBUG_VERBOSE(fmt, args...) do {                                        \
    if (g_rg_firmware_upgrade_debug) { \
        printk(KERN_INFO "[RG_FIRMWARE_UPGRADE][VER][func:%s line:%d]\r\n"fmt, __func__, __LINE__, ## args); \
    } \
} while (0)

#define RG_FIRMWARE_UPGRADE_DEBUG_ERROR(fmt, args...) do {                                        \
    if (g_rg_firmware_upgrade_error) { \
        printk(KERN_ERR "[RG_FIRMWARE_UPGRADE][ERR][func:%s line:%d]\r\n"fmt, __func__, __LINE__, ## args); \
    } \
} while (0)
...
static firmware_upgrade_device_t firmware_upgrade_device_data11 = {
    ...
    .en_logic_dev[2]     = "/dev/cpld1",
    .en_logic_addr[2]    = 0x3d,
    .en_logic_mask[2]    = 0xfe,
    .en_logic_en_val[2]  = 0x00,
    .en_logic_dis_val[2] = 0x01,
    .en_logic_width[2]   = 0x1,
...
static struct platform_device firmware_upgrade_device[] = {
 ...
 static int __init firmware_upgrade_device_init(void)
 {
     ...
     RG_FIRMWARE_UPGRADE_DEBUG_VERBOSE("enter!\n");
