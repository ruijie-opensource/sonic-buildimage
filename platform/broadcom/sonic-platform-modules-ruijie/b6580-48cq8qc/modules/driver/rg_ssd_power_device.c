...
static int g_rg_ssd_power_device_debug = 0;
static int g_rg_ssd_power_device_error = 0;

module_param(g_rg_ssd_power_device_debug, int, S_IRUGO | S_IWUSR);
module_param(g_rg_ssd_power_device_error, int, S_IRUGO | S_IWUSR);

#define RG_SSD_POWER_DEVICE_DEBUG(fmt, args...) do {                                        \
    if (g_rg_ssd_power_device_debug) { \
        printk(KERN_INFO "[RG_SSD_POWER_DEVICE][VER][func:%s line:%d]\r\n"fmt, __func__, __LINE__, ## args); \
    } \
} while (0)

#define RG_SSD_POWER_DEVICE_ERROR(fmt, args...) do {                                        \
    if (g_rg_ssd_power_device_error) { \
        printk(KERN_ERR "[RG_SSD_POWER_DEVICE][ERR][func:%s line:%d]\r\n"fmt, __func__, __LINE__, ## args); \
    } \
} while (0)
...
static void rg_ssd_power_device_release(struct device *dev)
{
    return;
}
...
static struct platform_device ssd_power_device[] = {
    {
        ...
        .dev = {
            ...
            .release = rg_ssd_power_device_release,
        },
    },
    {
        .name = "rg-ssd-power",
...
static int __init rg_ssd_power_device_init(void)
{
    int i, ret;
    ssd_power_device_t *ssd_power_device_data;

    RG_SSD_POWER_DEVICE_DEBUG("enter!\n");
    for (i = 0; i < ARRAY_SIZE(ssd_power_device); i++) {
        ...
        if (ret < 0) {
            ...
            printk(KERN_ERR "rg-ssd-power.%d device register failed\n", i + 1);
        } else {
...
static void __exit rg_ssd_power_device_exit(void)
{
    int i;
    ssd_power_device_t *ssd_power_device_data;
...
module_init(rg_ssd_power_device_init);
module_exit(rg_ssd_power_device_exit);
MODULE_DESCRIPTION("RG SSD POWER Devices");
MODULE_LICENSE("GPL");
